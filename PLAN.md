...
16. Added deep best-effort connect cancellation: `Cancel(connect-*)` marks the shared task, interrupts activation waits, and queues a target-guarded NetworkManager activation abort through the daemon runtime. The abort resolves the device's active-connection object, verifies its saved profile still matches the cancelled request's exact SSID bytes, and deactivates that captured object path; it skips deactivation if NetworkManager has already restored another profile. Already-sent synchronous D-Bus method calls cannot be interrupted mid-call.
...
26. Replaced per-subscription polling and per-operation watcher processes with one daemon-owned NetworkManager connection, bounded Tokio blocking lanes, one Tokio control actor, shared/coalesced subscription refreshes, cancellable task registrations, and coalesced background cache work. The frontend D-Bus service, JSONL transport, owner watcher, and bounded shutdown lifecycle also run on Tokio while NetworkManager workflows remain on the blocking domain boundary.
