...
| `wifi.profile.operation` | `{"operation":"set-casting","path":"/org/freedesktop/NetworkManager/Settings/1","enabled":true}` (`ProfileOperation`) | `result` | `—` | Mutates or builds a share payload for one saved Wi-Fi profile. |
...
Updates send the same basic fields plus an `advanced` object whose members are all optional. A field that is absent leaves the saved value untouched; an empty string or empty list clears a restriction, `mtu: 0` restores the automatic MTU, `channel: 0` clears a channel lock, and `band: "auto"` clears both band and channel. `cloned_mac_address` overrides `mac_address_policy` when both are sent. `advanced.casting_enabled` controls the profile's NetworkManager mDNS policy: enabled uses resolve-only mode so Cast devices can be discovered without registering the local hostname, while disabled blocks mDNS on that network. The shortcut operation `{"operation":"set-casting","path":"...","enabled":false}` updates the same setting. Profiles without an explicit mDNS value report casting as disabled, and connection through nm-daemon persists that disabled default instead of inheriting a potentially permissive system policy. The saved change applies on the next activation of an inactive profile; an already-active profile may need to reconnect before NetworkManager applies it. `mode` accepts `infrastructure`, `ap`, or `mesh` — ad-hoc is rejected because it has no modern secure ciphersuite — and `advanced.enterprise.eap` accepts only NetworkManager's supported EAP methods. Enterprise secrets continue to move through the existing top-level `secrets` map, never through `advanced.enterprise`.
